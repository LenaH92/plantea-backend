...
router.get('/blog/:blogPostId', async (req, res, next) => {
    ...
    if (mongoose.Types.ObjectId.isValid(blogPostId)) {
        try {
            const comments = await Comment.find({ blogPostId }).populate('userId', 'name email'); // Populate user info
            res.json(comments);
        } catch (error) {
            next(error);
        }
...
router.post('/', async (req, res, next) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(blogPostId) || !mongoose.Types.ObjectId.isValid(userId)) {
